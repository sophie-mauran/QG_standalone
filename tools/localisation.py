...
def localization_setup_cluster_obs(y2x_distances):
...
def nd_Id_localization_cluster_obs(shape,
                       obs_inds=None,
                       periodic=True):
    """Localize Id (direct) point obs of an N-D, homogeneous, rectangular domain."""
    M = np.prod(shape)
    ...
    return localization_setup_cluster_obs(y2x_distances)


def localization_setup_cluster_state(dist_xx):
    
    def localization_batch(radius, tag=None):
        def state_taperer(batch):
            dists = dist_xx[batch].min(axis=0)
            return inds_and_coeffs(dists, radius, tag=tag)
        return dist_xx, state_taperer
    return localization_batch

def nd_Id_cluster_loc_state(shape, periodic = True):
    M = np.prod(shape)
    ...
    return localization_setup_cluster_state(dist_xx)