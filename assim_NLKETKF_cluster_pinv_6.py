...
#        R_tilde=R_tilde/np.sqrt(2.0)
    ...
    #spy(K_cloc)
    K_cloc_HX = K_cloc[:M,M:]
    Ktmp = np.zeros((M,n_obs))
    for index, ii in enumerate(state_batches):
        if len(ii) != 0:
            K_cloc_H = np.zeros((n_obs,n_obs))
            ind_jj,tapering=obs_taperer(ii)
            Njj=len(ind_jj)
            for j in range(Njj):
                for j2 in range(j+1):
                    K_cloc_H[ind_jj[j],ind_jj[j2]] = (kernel(H_bar[:,ind_jj[j2]]*np.sqrt(tapering[j2]),H_bar[:,ind_jj[j]]*np.sqrt(tapering[j]),params_kernel))
            K_cloc_H = K_cloc_H + K_cloc_H.T - np.diag(K_cloc_H)
            Uc_H,Sigmac_H,_=np.linalg.svd(K_cloc_H,full_matrices='False',hermitian='True')

   
            ''' rg_K_loc_H=0
            for ll in range(n_obs):
                if(Sigmac_H[ll]>np.finfo(float).eps*Sigmac_H[0]):
                    rg_K_loc_H=rg_K_loc_H+1
                else:
                    break
            Uc_H=Uc_H[:,:rg_K_loc_H] '''
            for ll in range(n_obs):
                Uc_H[:,ll]=Uc_H[:,ll]/np.sqrt(Sigmac_H[ll]+N1)
            for i in range(len(ii)):
                
                Ktmp[ii[i],:] = K_cloc_HX[ii[i],:] @ Uc_H
